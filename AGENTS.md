...
It watches Esther's iA Writer drafts, picks the best one, runs a light copy-edit via Codex, gives Esther a 4-hour veto window by email, and publishes automatically if she doesn't object. It also watches already-published posts for edits and re-publishes them when the source file changes.
...
2. **Rank** — passes the top 10 candidates to Codex, which picks the best one for today
3. **Edit** — Codex runs a light copy-edit on the chosen draft (mechanical fixes only — typos, punctuation), returns a title, slug, and brief editorial note
...
**`rebuild` does NOT upload `style.css`.** It regenerates and uploads HTML only. If you changed the CSS, deploy it separately — see the next section.

### Deploy a style.css change

`style.css` is deliberately kept out of the build/upload flow: Esther fiddles with it often, and bundling it into `rebuild` would mean a full HTML re-upload every time she tweaks a color. So it ships on its own. After editing `assets/style.css`, push just that file:

```bash
cd "/Users/esther/Development/autopublish pipeline"
source venv/bin/activate
python -c "import yaml, os; from autopublish import publisher; cfg = yaml.safe_load(open('config.yaml')); publisher.sftp_upload_files(cfg, os.path.expanduser(cfg['site_path']), ['style.css'])"
```

`config.yaml`'s `site_path` may contain a literal `~` — hence the `os.path.expanduser`. The site's `style.css` is a symlink to `assets/style.css`, so `sftp.put` uploads the current repo version. No HTML rebuild is needed for a pure CSS change unless the markup classes also changed.
...
| `autopublish/ranker.py` | Codex ranking pass |
| `autopublish/editor.py` | Codex editorial pass |
...
- `style.css` — symlink to `assets/style.css` in this repo (source of truth). All styles including dark mode, a `@media (max-width: 880px)` block (index grid drops to 2 columns) and a `@media (max-width: 640px)` block for mobile (grid drops to 1 column). Not uploaded by `rebuild` or any pipeline job — deploy it separately after editing (see "Deploy a style.css change").
...
- Codex is called via the Codex CLI (`subprocess`). If Codex is unavailable, ranking and editing fall back to heuristics.
...
- **Inline formatting**: `**bold**`, `*italic*`, `__bold__`, `_italic_`, and `[label](url)` links. Links are extracted to placeholder tokens *before* the bold/italic regexes run so underscores/asterisks inside URLs (e.g. slugs, query strings) aren't parsed as emphasis. Inline markdown inside the link label is NOT processed — labels are treated as plain text.
- **Footnotes**: `[^footnote text]` inline — the text becomes the footnote body, rendered in a `<div class="footnotes">` at the bottom with numbered superscript links. **Footnotes may contain blank-line paragraph breaks** — `_extract_footnotes` runs on the raw text *before* `_split_blocks`, so blank lines inside `[^ ... ]` are preserved. Each paragraph inside a footnote renders as a separate `<p>` inside the `<li>`; the backlink (↩︎) lands at the end of the last paragraph. A literal `]` inside a footnote still terminates it (the regex is `[^\]]+`).
- **Blockquotes**: lines beginning with `>` (optional leading spaces tolerated). Consecutive non-empty `>` lines inside a paragraph are joined with `<br>` so poetry-style quotes preserve line breaks; empty `>` lines separate paragraphs within the same `<blockquote>`.
- **Leading spaces on list/blockquote lines are tolerated.** iA Writer sometimes prepends a single space to every list/quote line. `_LIST_LINE_RE` and `_BLOCKQUOTE_LINE_RE` allow `^ *` before the marker. Previously this caused a silent bug where only the *first* list item rendered: `_split_blocks` stripped the block's outer whitespace (so the first line matched the list regex), but subsequent lines kept their leading space and failed the per-line regex inside `_list_block_to_html`, so they were silently dropped.
...
## Standing instruction for Codex

When working on this codebase, update this AGENTS.md file whenever you discover something non-obvious about how the system works, fix a bug with a surprising root cause, or identify a behaviour that would confuse a future session. Do this without being asked.
